        ...
        for (i in 1..6) {
            ...
            val mButton = Button(this).also {
                ...
                it.setOnClickListener({
                    ...
                    if (checkBox.isChecked != correct) {
                        mProgress.progress += Math.ceil(100.toDouble() / 6).toInt()
                    }
                    checkBox.isChecked = correct
...
//        }
    ...
    private fun test6(): Boolean {
        val map1 = mapOf("A" to 2U, "B" to 4U, "C" to 3U)

        return task16(map1, mapOf("A" to 1U, "B" to 2U)) == 2U &&
                task16(map1, mapOf("F" to 1U, "G" to 2U)) == 0U &&
                task16(
                    mapOf("A" to 23U, "B" to 47U, "C" to 30U),
                    mapOf("A" to 1U, "B" to 2U, "C" to 4U)
                ) == 7U
    ...
    fun task16(store: Map<String, UInt>, asset: Map<String, UInt>): UInt {
        var res = UInt.MAX_VALUE
        for ((key, value) in asset) {
            val item = store.getOrDefault(key, 0U)
            res = (item / value).coerceAtMost(res)
        }
        return res
    }
}